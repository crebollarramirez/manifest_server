...
Operates purely on measured geometry (as produced by
``geometry_inspection.measure_geometry`` / ``execution_failed_geometry``, or
loaded back from a persisted ``geometry_snapshots`` row) -- never on source
text, semantic IDs, or any other CAD-intent metadata. All tolerances used to
decide "did this actually change" are centralized here rather than scattered
across callers.
"""
